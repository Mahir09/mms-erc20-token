let MmsToken = artifacts.require("./MmsToken.sol");

contract("MmsToken", (accounts) => {
  it("initializes the contract with correct values", async () => {
    let contract = await MmsToken.deployed();
    let name = await contract.name();
    let symbol = await contract.symbol();
    let standard = await contract.standard();

    assert.equal(name, "Mahir's Token", "Check Name of Token");
    assert.equal(symbol, "MMS", "Check Symbol of Token");
    assert.equal(standard, "Token v1.0", "Check Standard of Token");
  });
  ...
  it("Approves tokens for delegate transfer", async () => {
    const contract = await MmsToken.deployed();
